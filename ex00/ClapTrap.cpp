...
ClapTrap::ClapTrap() 
    ...
    energyPoints(10),
    attackDamage(0) {
  std::cout << "ClapTrap " << CL_YELLOW << name << CL_RESET << " constructed (default)" << std::endl;
}

ClapTrap::ClapTrap(const std::string& newName)
    : name(newName), hitPoints(10), energyPoints(10), attackDamage(0) {
  std::cout << "ClapTrap " << CL_YELLOW << name << CL_RESET << " constructed" << std::endl;
}
...
ClapTrap::ClapTrap(const ClapTrap& other)
  ...
  std::cout << "ClapTrap " << CL_MAGENTA << name << CL_RESET << " copy-constructed" << std::endl;
}
...
ClapTrap::~ClapTrap() {
  std::cout << "ClapTrap " << CL_YELLOW << name << CL_RESET << " Destructed" << std::endl;
}

void ClapTrap::attack(const std::string& target) {
  if (hitPoints == 0) {
    std::cout << "ClapTrap " << CL_RED << name << CL_RESET << " is dead!" << std::endl;
    return;
  }
  if (energyPoints == 0) {
    std::cout << "ClapTrap " << CL_RED << name << CL_RESET << " cannot "
              << CL_RED << "attack" << CL_RESET << ": no energy points left." << std::endl;
    return;
  }
  energyPoints--;
  std::cout << "ClapTrap " << CL_CYAN << name << CL_RESET << " "
...
void ClapTrap::beRepaired(unsigned int amount) {
  ...
  if (energyPoints > 0) {
    energyPoints--;
    hitPoints += amount;
    std::cout << "ClapTrap " << CL_GREEN << name << CL_RESET << " "
    ...
    std::cout << "ClapTrap " << CL_RED << name << CL_RESET
...
void ClapTrap::takeDamage(unsigned int amount) {
  if (hitPoints == 0) {
    std::cout << "ClapTrap " << CL_RED << name << CL_RESET << " is already dead!" << std::endl;
    return;
  }
  ...
  std::cout << "ClapTrap " << CL_RED << name << CL_RESET << " takes "
